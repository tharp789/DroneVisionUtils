...
# Drone Frame convention is x right, y down, z forward
# World Frame convention is ENU
# Lat is y, Long is x, Altitude is z
# yaw is in the opposite direction as the coordinate frame
# pitch is in the same direction as the coordinate frame
# image frame convention is x (u) right, y (v) down, theta is clockwise from x axis
# ------------------------------------------------------------------
...
class PointReprojection:
    ...
    def location_global_to_location_drone(self, drone_lat, drone_long, heading, home_lat, home_long, relative_altitude):
        ...
        relative_y_world, relative_x_world = self.lat_lon_distances_meters(drone_lat, drone_long, home_lat, home_long)
        ...
        # relative_x_world is east, relative_y_world is north, relative_altitude is up (so negate relative altitude which should always be positive)
    ...
    def get_pixel_from_location(self, drone_lat, drone_long, drone_altitude, gimbal_pitch, gimbal_yaw, zoom_level, heading, home_lat, home_long, home_altitude):
        ...
        home_in_drone = self.location_global_to_location_drone(drone_lat, drone_long, heading, home_lat, home_long, rel_alt)
        home_in_base = self.location_drone_to_location_base(home_in_drone)
        home_in_cam = self.location_base_to_location_camera(home_in_base, gimbal_pitch, gimbal_yaw)

        pixel, direction, pixel_inbounds = self.get_marker_location(home_in_cam, zoom_level, self.original_image_width / 2, self.original_image_height / 2)

        return pixel, direction, pixel_inbounds

    def get_marker_location(self, location_cam, zoom_level, center_u, center_v):
        '''
        Get the pixel location of a point in the camera frame
        
        Parameters:
        location_cam (numpy.ndarray): The location of the point in the camera frame (3,)
        zoom_level (int): The zoom level of the image
        center_u (int): The x pixel location of the center of the image
        center_v (int): The y pixel location of the center of the image
        
        Returns:
        tuple: The pixel location of the point in the image (u, v)
        float: The direction of the marker vector (theta)
        bool: Whether the pixel is in bounds

        '''
        pixel_u, pixel_v = self.get_pixel_from_location_camera(location_cam)
        ...
        if in_frame and not behind_camera:
            pixel_inbounds = True
            direction = None
            pixel = (pixel_u, pixel_v)
        else:
            pixel_inbounds = False
            pixel, direction = self.get_oob_marker_vector(pixel_u, pixel_v, center_u, center_v, behind_camera)
            
        return pixel, direction, pixel_inbounds

    def get_oob_marker_vector(self, pixel_u, pixel_v, center_u, center_v, behind_camera=False):
        '''
        ...
        '''
        # If the pixel is behind the camera, mirror it around center
        if behind_camera:
            pixel_u = center_u - (pixel_u - center_u)
            pixel_v = center_v - (pixel_v - center_v)
        ...
        if abs(dx) < 1e-9 and abs(dy) < 1e-9:
            # this case triggers if directily behind the camera
            # if direction is zero, return right most edge
            return (center_u * 2, 0), 0
    ...
    def get_pixel_from_location_camera(self, location_camera):
        ...
        # Project to 2D
        points_2d, _ = cv2.projectPoints(location_camera, rvec, tvec, self.calibration_matrix, self.distortion)
...
if __name__ == "__main__":
    # Example usage
    reprojection = PointReprojection('rgb1080_camera_calibration.npz', 1920, 1080)
